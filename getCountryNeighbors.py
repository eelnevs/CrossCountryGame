...
while play.lower() == "yes" or play.lower() == "y":
    ...
    start = travle_game.get_country_code_from_input(start_country)
    if start == -1:
        print("Country not found, please try again.")
        continue
    while True:
        end_country = input("Enter end country: ")
        end = travle_game.get_country_code_from_input(end_country)
        if end == -1:
            print("Country not found, please try again.")
            continue
        else:
            break
    country_path = travle_game.find_shortest_path(start, end)
